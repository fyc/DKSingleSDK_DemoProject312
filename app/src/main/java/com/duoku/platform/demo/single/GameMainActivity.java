...
//		DKCpWoStoreData woData = new DKCpWoStoreData("001");
		//初始化函数
		DKPlatform.getInstance().init(this, BuildConfig.isLandscape, DKPlatformSettings.SdkMode.SDK_PAY,initcompletelistener);
	}
...
//					Log.e("xxxx", "functionCode=" + mFunctionCode);
					...
					if(mFunctionCode == DkErrorCode.DK_ACCOUNT_LOGIN_SUCCESS){
						...
						mHandler.obtainMessage(DkErrorCode.DK_ACCOUNT_LOGIN_SUCCESS).sendToTarget();
//						Toast.makeText(GameMainActivity.this, "登录成功uid="+bduid+",token="+bdtoken+",oauthid="+bdoauthid, Toast.LENGTH_LONG).show();
						Toast.makeText(GameMainActivity.this, "登录成功", Toast.LENGTH_LONG).show();
	...
	Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
				case DkErrorCode.DK_ACCOUNT_LOGIN_SUCCESS:
					btnMyLogin.setVisibility(View.GONE);
					btnMyModify.setVisibility(View.VISIBLE);
		...
		public void onClick(View v) {
			switch(v.getId()){
				...
				case R.id.demoBtnSwitch:	
					...
					btnMyModify.setVisibility(View.GONE);
					btnMySwitchAccount.setVisibility(View.GONE);
					DKPlatform.getInstance().invokeBDChangeAccount(GameMainActivity.this, loginlistener);
					break;
	...
	protected void onResume() {
		super.onResume();
		if (!BDPlatformSDK.getInstance().isLogined(this)) {
			btnMyLogin.setVisibility(View.VISIBLE);
			btnMyModify.setVisibility(View.GONE);
			btnMySwitchAccount.setVisibility(View.GONE);
		}
